...
public class NoReSetDEL_160414000027
{
    ...
    public string NRS_DEL(DataTable parameter_forUI)
        {
            ...
            if (ht_forUI["yhbsp_session_uer_UAid"].ToString() == "U-A5E90103845E-703745D5-9E09-44A4-A029")
            {
                return "";
            }
                ...
                {
                    param.Add("@TID_" + d, delids[d]);

                    alsql.Add("delete ZZZ_M_TSD  where TID=@TID_" + d);

                //erp图片标记
                I_DBL.RunProc("UPDATE  View_M_MOB_view_FKBG SET ffj = 0 WHERE   Number = (SELECT   TOP 1 TSDH  FROM      ZZZ_M_TSD  WHERE   TID = '"+ delids[d] + "')");
