...
public class NoReSetDEL_160713000065
{
        ...
        if (ht_forUI.Contains("zdyname") && ht_forUI["xuanzhongzhi"].ToString() != "")
        {
            ...
            for (int d = 0; d < ids.Length; d++)
            {
                if (ids[d].Trim() != "")
                {
                    ...
                    alsql.Add("UPDATE ZZZ_xiaoshoufahuo SET  FCzhuangtai='已收件',FCqurenshoujianren=@FCqurenshoujianren,FCshoujianshijian=getdate()  where FCzhuangtai='在途' and FCID =@FCID_" + d);
                }
