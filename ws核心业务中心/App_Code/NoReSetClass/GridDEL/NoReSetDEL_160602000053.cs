...
public class NoReSetDEL_160602000053
{
    ...
    public string NRS_ZDY_querenshouhuo(DataTable parameter_forUI)
    {
        ...
        if (ht_forUI.Contains("zdyname") && ht_forUI["xuanzhongzhi"].ToString() != "")
        {
            ...
            param.Add("@rqurenren", ht_forUI["yhbsp_session_uer_UAid"].ToString());
            for (int d = 0; d < ids.Length; d++)
            {
                if (ids[d].Trim() != "")
                {
                    ...
                    alsql.Add("UPDATE ZZZ_C_record SET  rzhuangtai='已收件',rqurenren=@rqurenren,rshoujianshijian=getdate()  where rzhuangtai='在途' and rid =@rid_" + d);
                }
