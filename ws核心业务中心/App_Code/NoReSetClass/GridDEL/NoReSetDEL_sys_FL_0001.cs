            ...
            for (int d = 0; d < ids.Length; d++)
            {
                if (ids[d].Trim() != "")
                {
                    ...
                    for (int i = 0; i < DTsub.Rows.Count; i++)
                    {
                        ...
                        alsql.Add("insert into FUP_FormsList_field select 'xx_"+ guid_sub + "' as DID, '"+ guid + "' as DID_FSID, DID_ok, DID_px, DID_hide, DID_showname, DID_name, DID_width, DID_sortable, DID_fixed,    DID_frozen, DID_formatter, DID_formatter_CS  from FUP_FormsList_field where DID='" + DTsub.Rows[i]["DID"].ToString() + "'");
                    }
                }
