<%@ Page Title="" Language="C#" MasterPageFile="~/adminht/MasterPageMain.master" AutoEventWireup="true" CodeFile="list_record_main_my.aspx.cs" Inherits="lpjgl_list_record_main_my" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="sp_script" runat="Server">
    ...
    <script type="text/javascript">
       ...
       $("#zheshiliebiaoquyu").attr('teshuwhere', "rid in (select distinct rid from View_ZZZ_C_record_sub_ex where wh_chu_dpname = (select top 1 xingming from ZZZ_userinfo where UAid='<%=UserSession.唯一键%>') or wh_ru_dpname=(select top 1 xingming from ZZZ_userinfo where UAid='<%=UserSession.唯一键%>') )");
